...
* category.coding ^slicing.discriminator.path = "system" //katsetus - enne oli $this ja value
* category.coding ^slicing.rules = #open
* category.coding ^short = "RenBase drugFormGroup ja failureDegree"
* category.coding contains
    drugFormGroup 0..1 and
    failureDegree 0..1 and
    additionalInformation 0..1
* category.coding[drugFormGroup] ^short = "0 | 1 | 2 | 3 | 4 | 5| This is a fake CS for example. Do NOT use it!"
* category.coding[drugFormGroup] from $drug-form-group-VS (preferred)
//* category.text[drugFormGroup] = "Filled with information from RenBase. Renbase drug form for which recommendations should be provided. E.g Systemic, parenteral etc"
...
* category.coding[failureDegree] ^short = "Failure degree 0-4. Neerupuudulikkuse aste: number + selgitus. This is a fake CS for example. Do NOT use it! Sellist CS ei eksisteeri! testimiseks."
* category.coding[failureDegree] from $failure-degree-VS (preferred) //see on siia kunstlikult pandud. ^binding.description = "siia miskit????KAS TULEB CS?"
...
* category.coding[additionalInformation] from $additional-information-VS (preferred) //^binding.description = "siia miskit????KAS TULEB CS?"
* category.coding[additionalInformation] ^short = "This is a fake CS for example. Do NOT use it! Sellist CS ei eksisteeri! testimiseks."
